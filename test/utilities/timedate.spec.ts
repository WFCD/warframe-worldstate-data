...
describe("timeDateUtils", () => {
  describe("timeDeltaToString()", () => {
    ...
    it("shows seconds, minutes, hours and days if the difference is more than a day", () => {
      timeDeltaToString(120000000).should.match(
    ...
    it("should test non-number inputs", () => {
      (() => timeDeltaToString("poop" as unknown as number)).should.throw(
        TypeError,
      );
    });
  ...
  describe("parseDate()", () => {
    it("should parse even if date provided is undefined", () => {
      ...
      (() =>
        parseDate(
          2340985790347890 as unknown as ContentTimestamp,
        )).should.not.throw();
    });
  });
  ...
  describe("pieceIsSmoller()", () => {
    ...
    it("should correctly extract time pieces", () => {
      ...
      ({ seconds, timePieces } = pieceIsSmoller(
        seconds,
        86400,
        "d",
        timePieces,
